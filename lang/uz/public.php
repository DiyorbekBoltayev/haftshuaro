<?php
return [
    //navbar uchun
    "local"=>"UZ",
    "bosh_sahifa"=>"Bosh sahifa",
    "bosh_sahifa2"=>"Sayt mulliflari haqida",
    "bosh_sahifa3"=>"Fotogalereya",

    "adabiy_muhit"=>"Adabiy muhit",
    "adabiy_muhit2"=>"Adabiy muhit va badiiy ijod",
    "adabiy_muhit3"=>"Biografik ma'lumotlar",
    "adabiy_muhit4"=>"\"Haft shuaro\"ning nusxalari",

    "gazallar"=>"G'azallar",
    "gazallar2"=>"G'azallar tasnifi",
    "gazallar3"=>"She'riy san'at",

    "sh_janr"=>"She'riy janrlar",
    "sh_janr2"=>"Musammat",
    "sh_janr3"=>"Boshqa janrlar",

    "sheriyat"=>"She'riyat",
    "sheriyat2"=>" ijodi",

    "kutubxona"=>"Kutubxona",
    "kutubxona2"=>"Durdona to'plamlar",
    "kutubxona3"=>"Eng sara asarlar",

    //har bir sahifadagi headerlar
    "t_1_1"=>"“Haft shuaro” majmuasini o'rganish orqali XIX asr Xorazm vohasining milliy-ma'naviy merosini tadqiq qilish ",
    "t_1_2"=>"Sayt mulliflari haqida",
    "t_1_3"=>"Fotogalereya",

    "t_2_1"=>"XX asr boshidagi Xorazm madaniy- adabiy muhitining asosiy xususiyatlari",
    "t_2_2"=>"Adabiy muhit va badiiy ijod qirralari",
    "t_2_3"=>"Manbalardagi biografik ma`lumotlarning adabiy qimmati",
    "t_2_4"=>"“Haft shuaro”ning nusxalari va tuzilishi",
    't_2_5'=>"Tadqiqotlar va maqolalar",
    't_2_6'=>"Ijodkorlar",
    'menu_tad'=>"Tadqiqotlar",
    'footer_text'=>"Mualliflik huquqi 2022 Haft Shuaro. Barcha huquqlar himoyalangan.",
    'see_all'=>"Barchasini ko'rish",
    "j_azo"=>"Jamoamiz a'zolari",
    "ijod_n_t"=>"Ijod namunalari bilan tanishish"



//davomi borrrrr




]
?>
